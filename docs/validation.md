...
Release 0.2.0 provides a specified algorithm, package, examples and semantic regression tests. It is an experimental method. No multi-language human-validation study or peer-reviewed novelty claim is included.
...
2. **Human construct validity:** Ask reviewers whether each proposed transcript switch preserves the language direction and location, then separately whether its local words are correct. Include long inserted prefixes, boundary-adjacent insertions and repeated passages. Compare boundary F1 with switch judgments and anchored F1 with local-transcription judgments. Use a held-out portion to assess correlation, rather than selecting rules and demonstrating agreement on the same examples.
...
“We report aligned language-switch boundary F1 (`switchf1-boundary-v1`), a proposed text-based metric requiring correct directed transitions at the same pair of edit-alignment positions, without requiring exact neighboring words. We pool event counts across all test utterances, including monolingual references. We also report WER/CER and optionally anchored F1 as a stricter transcription diagnostic. Language labels were obtained by [describe independently validated procedure].”
